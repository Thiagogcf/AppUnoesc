...
ADMIN_PASSWORD = generate_password_hash('unoesc')
...
def submit():
    ...
    problem_solution = request.form.get('problem_solution', '')
        ...
        with open('respostas.csv', 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                submissions.append({
                    ...
                    'escola': row[3],
                    'respostas': row[4:10],
                    'problema': row[10] if row[10] else 'Não informado',
                    'imagem': row[11] if row[11] else 'Não enviada',
                    'solucao': row[12] if row[12] else 'Não informada',
